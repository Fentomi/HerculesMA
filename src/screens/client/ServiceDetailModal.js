...
const ServiceDetailModal = ({ visible, onClose, serviceId, clientId }) => {
  ...
  const loadDetail = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await fetch(`${API_URL}/clients/${clientId}/services/${serviceId}`);
      ...
      <SafeAreaView style={styles.overlay}>
        <View style={styles.modal}>
          <View style={styles.header}>
            <Text style={styles.title}>Детали услуги</Text>
          ...
          {loading ? (
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
              <View style={styles.detailCard}>
                <Text style={styles.serviceName}>{detail.service_name}</Text>
                {detail.service_description ? (
                  <Text style={styles.description}>{detail.service_description}</Text>
                ) : null}
                <Text style={styles.price}>{detail.current_price} ₽</Text>

                <DetailRow label="Дата покупки" value={formatDate(detail.buy_date)} />
                <DetailRow label="Осталось занятий" value={detail.remaining_uses} />
                <DetailRow label="Способ оплаты" value={detail.payment_method} />
                <DetailRow label="Продавец" value={detail.seller_name} />
                {detail.trainer_name ? (
                  <DetailRow label="Тренер" value={detail.trainer_name} />
                ) : null}
              </View>
...
const styles = StyleSheet.create({
  ...
  price: { fontSize: 18, color: '#4F46E5', fontWeight: '600', marginBottom: 12 },
  detailRow: {
    flexDirection: 'row',
    justifyContent: 'space-between',
    marginBottom: 6,
  ...
  noVisits: { textAlign: 'center', color: '#9CA3AF', marginTop: 20 },
  ...
  pageButtonText: { color: '#fff', fontSize: 14, fontWeight: '500' },
  pageButtonTextDisabled: { color: '#9CA3AF' },
  pageIndicator: { fontSize: 14, color: '#6B7280', fontWeight: '500' },
});
